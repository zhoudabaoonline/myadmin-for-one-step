...
def get_department_exclude_id(*, db: Session = Depends(deps.get_db), id: int, ) -> Any:
    """返回一个排除了子节点的树结构"""
    departments = db.query(models.Department).all()
    departments = list_to_tree([department.dict()
                               for department in departments], order="order", exclude=id)
    return {"code": 20000, "data": departments, }
...
def update_department(*, db: Session = Depends(deps.get_db), department: schemas.DepartmentUpdate) -> Any:
    ...
    db.query(models.Department).filter(
        models.Department.id == department.id).update(department)
    db.commit()
...
def delete_department_id(department_id: int, db: Session = Depends(deps.get_db), ) -> Any:
    """部门管理-删除"""
    users = db.query(models.User_Department).filter(
        models.User_Department.department_id == department_id).all()
    if users != []:
        raise HTTPException(status_code=200, detail="该部门下存在员工，禁止删除")

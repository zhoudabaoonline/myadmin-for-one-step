...
def read_users(*,
    ...
    total = query.count()
    users = query.order_by(models.User.username).limit(
    ...
    try:
        ...
        for row in ws.iter_rows(min_row=2):
            ...
            if not exist_user.first():
                ...
                user_department = {"user_id": exist_user_id,
                                   "department_id": department.id}
                db.add(models.User_Department(**user_department))
                # post
                db.query(models.User_Dict).filter(
                    models.User_Dict.user_id == exist_user_id).delete()
                user_dict = [{"user_id": exist_user_id, "dict_id": post.id}
                             for post in posts]
                db.bulk_insert_mappings(models.User_Dict, user_dict)
        db.commit()
        ...
        raise HTTPException(
            status_code=200, detail=f"导入失败，请检查数据!   Error Reason: {exc}")
    finally:
        wb.close()

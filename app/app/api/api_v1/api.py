...
# 各自模块的路由由各自模块负责
api_router.include_router(endpoints.router, dependencies=[
                          Depends(deps.get_current_active_user)])
api_router.include_router(system.router, dependencies=[
                          Depends(deps.get_current_active_user)])
api_router.include_router(report.router, dependencies=[
                          Depends(deps.get_current_active_user)])
api_router.include_router(product.router, dependencies=[
                          Depends(deps.get_current_active_user)])
